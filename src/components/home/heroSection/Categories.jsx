                            ...
                            <ul className='absolute left-full bg-white w-full top-0 hidden group-hover:block py-2 h-full z-50'>
                                {
                                    category?.children?.map(subCat => subCat?.children?.length > 0 ?
                                        <li key={subCat?._id} className='group/sub'>
                                            <Link href={`/products?category=${subCat?.slug}`} className='flex items-center justify-between pl-4 py-1 text-xs xl:text-sm text-dark hover:bg-gray-100'> <span className='line-clamp-1'>{subCat?.title}</span> <IoIosArrowForward className='text-primary mr-4' /></Link>
                                            {/* sub menu child  */}
                                            <ul className='absolute left-full bg-white w-full top-0 hidden group-hover/sub:block py-2 h-full'>
                                                {
                                                    subCat?.children?.map(subCatChild =>
                                                        <li key={subCatChild?._id}>
                                                            <Link href={`/products?category=${subCatChild?.slug}`} className='flex items-center justify-between pl-4 py-1 text-xs xl:text-sm text-dark hover:bg-gray-100'> <span className='line-clamp-1'>{subCatChild?.title}</span></Link>
                                                        </li>
                                                    )
                                                }
                                            </ul>
                                        ...
                                        <li key={subCat?._id}>
                                            <Link href={`/products?category=${subCat?.slug}`} className='flex items-center justify-between pl-4 py-1 text-xs xl:text-sm text-dark hover:bg-gray-100'> <span className='line-clamp-1'>{subCat?.title}</span></Link>
                                        </li>
                                    )
                                }
