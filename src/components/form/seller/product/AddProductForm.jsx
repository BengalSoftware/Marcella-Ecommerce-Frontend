...
} from "@/lib/productApi/productApi";
...
const AddProductForm = ({ id }) => {
  ...
  const handleUpdate = async (event) => {
    ...
    if (updateProduct?.productType)
      formData.append("productType", updateProduct?.productType);
    ...
    if (updateProduct?.subcategories)
      formData.append("subcategories", updateProduct?.subcategories);
    if (updateProduct?.subcategoryChildren)
      formData.append(
        "subcategoryChildren",
        updateProduct?.subcategoryChildren,
      );
    if (singleSeller) formData.append("sellerId", singleSeller);

    // form data.append JSON.stringify(data)
    if (selectedSizeOption)
      formData.append("size", JSON.stringify(selectedSizeOption));
    if (selectedColorOption)
      formData.append("color", JSON.stringify(selectedColorOption));
    ...
    if (id) {
      ...
      //     setSellerPSuccess(true);
      //   }
      console.log(res); // to see bug adding products in seller page
    }
  };
    ...
    <div className="bg-white p-4 shadow rounded-md mt-5">
      <form onSubmit={handleUpdate}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          <div>
            ...
            <input
              ...
              name="name"
              required
              className="border mt-2 border-gray-300 outline-none p-2 w-full block rounded-md placeholder:text-sm placeholder:font-light"
          ...
          <div>
            ...
            <div className="flex items-center border border-gray-300 mt-2 rounded-md">
              <input
                onChange={handleChange}
                defaultValue={slug ? slug : generateSlug}
                type="text"
                name="slug"
                required
                className="outline-none rounded-l-md p-2 w-full placeholder:text-sm placeholder:font-light"
                placeholder="Slug"
              />
              <button
                onClick={handleGenerateSlug}
                type="button"
                className="text-sm text-white rounded-r-md bg-primary py-2.5 px-1 hover:bg-dark"
              >
                Generate
              </button>
            </div>
          ...
          <BrandForm handleChange={handleChange} manufacturer={manufacturer} />
          ...
          <div>
            ...
            <select
              ...
              onChange={handleChange}
              name="status"
              className="block w-full border rounded-md p-2.5 mt-2 outline-none text-dark text-sm"
            >
        ...
        <ProductDescriptionForm
          ...
          handleChange={handleChange}
        />
        <UploadImage upImages={images} setUpImages={setUpImages} />
        <div className="flex justify-end mt-5">
          <button className="bg-primary hover:bg-dark px-6 py-2 text-white rounded-md">
            {id ? "Update" : "Submit"}
          </button>
        </div>
      </form>
